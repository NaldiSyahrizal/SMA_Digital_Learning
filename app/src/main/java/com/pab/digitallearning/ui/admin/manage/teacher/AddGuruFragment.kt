...
import com.pab.digitallearning.databinding.FragmentTambahGuruBinding
...
class AddGuruFragment : Fragment() {
    ...
    private var _binding: FragmentTambahGuruBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentTambahGuruBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun setupListeners() {
        ...
        binding.btnSimpan.setGemoyClick { saveTeacher() }
    }
    ...
    private fun saveTeacher() {
        ...
        if (username.isEmpty() || email.isEmpty() || password.isEmpty() || nip.isEmpty() || nama.isEmpty() || jk.isEmpty()) {
        ...
        ApiClient.apiService.addTeacher(username, email, nama, nip, jk, telp, password).enqueue(object : Callback<BasicResponse> {
            override fun onResponse(call: Call<BasicResponse>, response: Response<BasicResponse>) {
                if (_binding == null) return
                binding.btnSimpan.isEnabled = true
                if (response.isSuccessful) {
                    com.pab.digitallearning.util.AdminActivityTracker.logActivity(requireContext(), "Menambahkan guru baru: $nama", "add")
                    DialogUtils.showSuccessDialog(requireContext(), message = "Guru berhasil ditambahkan") {
                        findNavController().popBackStack()
                    }
                } else {
                    val errorBody = response.errorBody()?.string()
                    DialogUtils.showErrorDialog(requireContext(), errorBodyString = errorBody, fallbackMessage = "Gagal menambah guru")
                }
            }
