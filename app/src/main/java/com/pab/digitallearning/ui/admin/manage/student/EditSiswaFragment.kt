...
import com.pab.digitallearning.databinding.FragmentEditSiswaBinding
...
class EditSiswaFragment : Fragment() {

    private var _binding: FragmentEditSiswaBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEditSiswaBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun setupInitialData() {
        studentId = arguments?.getLong("id") ?: 0
        binding.etNama.setText(arguments?.getString("nama"))
        binding.etNis.setText(arguments?.getString("nis"))
    ...
    private fun enableEditMode(enable: Boolean) {
        ...
        binding.etNis.setTextColor(textColor)
        ...
        binding.etNis.isEnabled = enable
    ...
    private fun updateStudent() {
        ...
        if (nama.isEmpty() || nis.isEmpty() || fullGender.isEmpty() || username.isEmpty() || email.isEmpty()) {
        ...
        showLoading(true)
        ApiClient.apiService.updateStudent(studentId, username, email, nama, nis, genderCode, telp, passwordParam).enqueue(object : Callback<BasicResponse> {
            override fun onResponse(call: Call<BasicResponse>, response: Response<BasicResponse>) {
                if (_binding == null) return
                binding.btnActionToggle.isEnabled = true
                showLoading(false)
                if (response.isSuccessful) {
                    com.pab.digitallearning.util.AdminActivityTracker.logActivity(requireContext(), "Mengubah data siswa: $nama", "edit")
                    DialogUtils.showSuccessDialog(requireContext(), message = "Siswa berhasil diupdate") {
                        findNavController().popBackStack()
                    }
                } else {
                    val errorBody = response.errorBody()?.string()
                    DialogUtils.showErrorDialog(requireContext(), errorBodyString = errorBody, fallbackMessage = "Gagal update siswa")
                }
            }
